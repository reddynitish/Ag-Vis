from ag_vis.cli import _poll_once, build_parser
...
def test_new_session_resets_completed_progress(tmp_path):
    ...
    store = StateStore()
    cursors = {}
    active = _poll_once(store, root, workspace, cursors, None, None)
    assert store.snapshot()[1].progress == 1

    second = root / "second.jsonl"
    second.write_text(f'{{"cwd":"{workspace}","type":"turn_started"}}\n')
    active = _poll_once(store, root, workspace, cursors, active, None)
