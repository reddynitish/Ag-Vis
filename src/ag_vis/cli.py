...
def _seed_cursors(root: Path, workspace: Path) -> tuple[dict[Path, TailCursor], Path | None]:
    sessions = discover_sessions(root, workspace)
    cursors: dict[Path, TailCursor] = {}
    for path in sessions:
        try:
            stat = path.stat()
            cursors[path] = TailCursor(stat.st_size, (stat.st_dev, stat.st_ino))
        except OSError:
            continue
    return cursors, sessions[-1] if sessions else None


def _watch(store: StateStore, root: Path, workspace: Path, use_laya: bool = False, on_prompt=None) -> None:
    stop = threading.Event()
    cursors, active = _seed_cursors(root, workspace)
    fallback = classify_with_laya if use_laya else None
    while not stop.is_set():
        active = _poll_once(store, root, workspace, cursors, active, fallback, on_prompt)
...
def _open_popup(url: str) -> None:
    ...
    try:
        if sys.platform == "darwin" and os.path.exists(chrome):
            subprocess.Popen(
                ["open", "-na", "Google Chrome", "--args", f"--app={url}", "--window-size=1180,760"],
...
def run(args: argparse.Namespace) -> int:
    ...
    if args.demo:
        ...
        on_prompt = None if args.no_open else lambda: (None if store.has_clients() else _open_popup(url))
        threading.Thread(
            target=_watch,
            args=(store, args.session_root, args.workspace.resolve(), args.laya, on_prompt),
            daemon=True,
        ).start()
    if args.demo and not args.no_open:
        _open_popup(url)
